...
class InvitationFromOwner_crud:
    ...
    async def get_invitation_by_id(self, id: int):
        invitation = await self.db.fetch_one(invitations_from_owner.select().where(invitations_from_owner.c.id == id))
        if invitation == None:
            return None
    ...
    async def get_invitation_by_invented_id(self, id: int):
        ...
        list_invitation = await self.db.fetch_all(query=query)
