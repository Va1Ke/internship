...
class QuizQuestionAnswer:
    ...
    async def add_answer(self, answer: quiz_question_answers_schemas.QuizQuestionAnswerEntry, db: databases.Database) -> quiz_question_answers_schemas.QuizQuestionAnswerReturn:
    ...
    async def delete_answer_for_question_for_crud(self, question_id: int, db: databases.Database) -> HTTPException:
        query = quiz_answers.delete().where(quiz_answers.c.question_id == question_id)
        await db.execute(query=query)
        return HTTPException(status_code=200, detail="Success")

    async def delete_answer(self, answer_id: int, question_id: int, db: databases.Database) -> HTTPException:
