...
class RequestFromUser_crud:
    ...
    async def create_request(self, company_id: int, user_id: int, db: databases.Database) -> requst_from_user_schemas.UserRequestReturn:
    ...
    async def get_request_by_id(self, request_id: int, db: databases.Database) -> requst_from_user_schemas.UserRequestReturn:
    ...
    async def accept_request_from_user(self, request_id: int, company_id: int, user_id: int, db: databases.Database) -> invitation_from_owner_schemas.UserCompanyReturn:
    ...
    async def decline_request_from_user(self, request_id: int, db: databases.Database) -> HTTPException:
        query = requests_from_user.delete().where(requests_from_user.c.id == request_id)
        await db.execute(query=query)
