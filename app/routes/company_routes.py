...
async def create(company: Company, email: str = Depends(get_email_from_token)):
    ...
    return await company_crud.create_company(company=company, owner_id=owner.id,db=db)
...
async def add_admins(company_id: int, user_id: int, email: str = Depends(get_email_from_token)):
    ...
    if company.owner_id == owner.id:
        return await company_crud.add_to_admins_in_company(company_id=company_id, user_id=user_id,db=db)
    else:
...
async def show_requests_from_users(company_id: int, email: str = Depends(get_email_from_token)):
    ...
    company = await company_crud.get_company_by_id(company_id,db=db)
    if company.owner_id == owner.id:
        return await company_crud.show_requests_from_users(company_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def accept_request_from_user(request_id: int, email: str = Depends(get_email_from_token)):
    owner = await user_crud.get_user_by_email(email,db=db)
    request = await req_from_user_crud.get_request_by_id(request_id,db=db)
    company = await company_crud.get_company_by_id(request.company_id,db=db)
    if company.owner_id == owner.id:
        return await req_from_user_crud.accept_request_from_user(request_id=request_id, company_id=company.id, user_id=request.user_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def update(company: UpdateCompany, email: str = Depends(get_email_from_token)):
    owner = await user_crud.get_user_by_email(email,db=db)
    company_get = await company_crud.get_company_by_id(company.id,db=db)
    if company_get.owner_id == owner.id:
        return await company_crud.update_company(company=company,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def delete(company_id: int, email: str = Depends(get_email_from_token)):
    ...
    company = await company_crud.get_company_by_id(company_id,db=db)
    if company.owner_id == owner.id:
        return await company_crud.delete_company(company_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def delete_user(company_id: int, user_id: int, email: str = Depends(get_email_from_token)):
    ...
    company = await company_crud.get_company_by_id(company_id,db=db)
    if company.owner_id == owner.id:
        return await company_crud.delete_user_from_company(company_id=company_id, user_id=user_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def get_company_by_owner(email: str = Depends(get_email_from_token)):
    user_exist = await user_crud.get_user_by_email(email,db=db)
    if user_exist:
        return await company_crud.get_company(user_exist.id,db=db)
    else:
...
async def show_companies():
    return await company_crud.get_companies(db=db)
