...
async def create(quiz: QuizEntry, email: str = Depends(get_email_from_token)) -> QuizReturn:
    ...
    check_is_admin = await company_crud.check_is_admin(company_id=quiz.company_id, user_id=owner.id,db=db)
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_crud.create_quiz(quiz,db=db)
    else:
...
async def get_quizzes_by_company(company_id: int, email: str = Depends(get_email_from_token)) -> list[QuizReturn]:
    ...
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_crud.show_quizzes_in_company(company_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def create(quiz_entry: QuizUpdateEntry, email: str = Depends(get_email_from_token)) -> QuizReturn:
    owner = await user_crud.get_user_by_email(email,db=db)
    quiz = await quiz_crud.get_quiz_by_id(quiz_entry.id,db=db)
    check_is_admin = await company_crud.check_is_admin(company_id=quiz.company_id, user_id=owner.id,db=db)
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_crud.update_quiz(quiz_entry,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def create(quiz_id: int, email: str = Depends(get_email_from_token)):
    ...
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_crud.delete_quiz(quiz_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def add_question(question: quiz_questions_schemas.QuizQuestionEntry, email: str = Depends(get_email_from_token)) -> quiz_questions_schemas.QuizQuestionReturn:
    owner = await user_crud.get_user_by_email(email,db=db)
    quiz = await quiz_crud.get_quiz_by_id(question.quiz_id,db=db)
    check_is_admin = await company_crud.check_is_admin(company_id=quiz.company_id, user_id=owner.id,db=db)
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_questions_crud.add_question(question,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def create(quiz_id: int, question_id: int, email: str = Depends(get_email_from_token)):
    ...
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_questions_crud.delete_question(quiz_id=quiz_id,question_id=question_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def add_question(answer: quiz_question_answers_schemas.QuizQuestionAnswerEntry, email: str = Depends(get_email_from_token)) -> quiz_question_answers_schemas.QuizQuestionAnswerReturn:
    owner = await user_crud.get_user_by_email(email,db=db)
    question = await quiz_questions_crud.get_question_by_id(answer.question_id,db=db)
    quiz = await quiz_crud.get_quiz_by_id(question.quiz_id,db=db)
    check_is_admin = await company_crud.check_is_admin(company_id=quiz.company_id, user_id=owner.id,db=db)
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_questions_answers_crud.add_answer(answer,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")
...
async def create(answer_id: int, question_id: int, email: str = Depends(get_email_from_token)):
    owner = await user_crud.get_user_by_email(email,db=db)
    question = await quiz_questions_crud.get_question_by_id(question_id,db=db)
    quiz = await quiz_crud.get_quiz_by_id(question.quiz_id,db=db)
    check_is_admin = await company_crud.check_is_admin(company_id=quiz.company_id, user_id=owner.id,db=db)
    company = await company_crud.get_company_by_id(quiz.company_id,db=db)
    if company.owner_id == owner.id or check_is_admin == True:
        return await quiz_questions_answers_crud.delete_answer(answer_id=answer_id, question_id=question_id,db=db)
    else:
        raise HTTPException(status_code=400, detail="No permission")