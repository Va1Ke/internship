...
async def create(company_id: int, email: str = Depends(get_email_from_token)):
    user = await user_crud.get_user_by_email(email,db=db)
    return await req_from_user_crud.create_request(company_id=company_id, user_id=user.id,db=db)
...
async def delete_request(request_id: int, email: str = Depends(get_email_from_token)):
    user = await user_crud.get_user_by_email(email,db=db)
    request = await req_from_user_crud.get_request_by_id(request_id=request_id,db=db)
    company = await company_crud.get_company_by_id(company_id=request.company_id,db=db)
    if company.owner_id == user.id:
        return await req_from_user_crud.decline_request_from_user(request_id,db=db)
    else:
