...
async def create_user(user: SignUpUser):
    db_user = await UserCrud(db=db).get_user_by_email(email=user.email)
    if db_user:
        raise HTTPException(status_code=400, detail="Email already registered")
    return await UserCrud(db=db).create_user(user=user)

@router.put("/update-user/",response_model=User)
async def update_user(user: UserUpdate, email: str = Depends(get_email_from_token)):
    if user.email == email:
        return await UserCrud(db=db).update_user(user=user)
    else:
...
async def delete_user(user: UserBase, email: str = Depends(get_email_from_token)):
    if user.email == email:
        return await UserCrud(db=db).delete_user(user=user)
    else:
        raise HTTPException(status_code=400, detail="No such user or you have no permissions to do that")

@router.get("/users/{user_id}", response_model=User)
async def read_user(user_id: int):
    db_user = await UserCrud(db=db).get_user_by_id(id=user_id)
    if db_user is None:
...
async def read_user_by_email(user_email: str):
    db_user = await UserCrud(db=db).get_user_by_email(email=user_email)
    if db_user is None:
        raise HTTPException(status_code=400, detail="No such user")
    return db_user
...
async def sign_in_my(user: SignInUser):
    user_check = await UserCrud(db=db).get_user_by_email(user.email)
...
async def sign_up_my(user: SignUpUser):
    does_exist = await UserCrud(db=db).get_user_by_email(email=user.email)
    if does_exist:
        raise HTTPException(status_code=400,detail="Email already registered")
    config = set_up()
    ...
    user = await UserCrud(db=db).create_user(user)
    access_token_expires = timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
    token = await create_access_token(user.email, expires_delta=access_token_expires)
    return token
